...
class PlayState extends MusicBeatState
{
	...
	private var DrawOver:FlxSprite;
	...
	public var additionalHealth:FlxSprite;
	...
	public var bgArray:Array<FlxSprite> = [];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'SenHouse':
				camGame.maxScrollY = 750;
				var bg:BGSprite = new BGSprite('SenSky', -400, -800, 0.9, 0.9);
				bg.scale.set(1.2,1.2);
				add(bg);

				var stageFront:BGSprite = new BGSprite('SenGround', -1250, -800, 0.95, 0.95);
				stageFront.scale.set(1,1);
				
				//stageFront.setGraphicSize(Std.int(stageFront.width));
				stageFront.updateHitbox();
				add(stageFront);
				
				additionalHealth = new FlxSprite(0, 0);
				
				additionalHealth.scale.set(.9,.9);
				additionalHealth.frames = Paths.getSparrowAtlas('SENAnimatedAssets/HeartBar');
				snesHealth = 1;
				additionalHealth.updateHitbox();
				additionalHealth.screenCenter(X);
				for (i in 1...11){
					additionalHealth.animation.addByPrefix('Health' + i, 'health' + (i-1), 1 , true);
				}
				additionalHealth.animation.play('Health' + snesHealth);
				additionalHealth.y = 0.6 * FlxG.height;
				additionalHealth.cameras=[camHUD];
				bgArray[0] = bg;
				bgArray[1] = stageFront;
			case 'Glitchy':
				var bg:FlxSprite = new FlxSprite(-800, -500);
				bg.scale.set(1.5, 1.5);
				
				bg.frames = Paths.getSparrowAtlas('GlitchRoom');
				bg.animation.addByPrefix('beat', "beat", 1, true);
				bg.animation.play('beat');
				beatBG = bg;
				add(beatBG);
				
				bgArray[3] = bg;
				
				var fakeBg:BGSprite = new BGSprite('mountains', -2000, -1500, 1.2, 1.2);
				add(fakeBg);
				bgArray[0] = fakeBg;
				var stageFake:BGSprite = new BGSprite('falseTriforce', -2200, -1450, 1, 1);
				stageFake.setGraphicSize(Std.int(stageFake.width * 1.1));
				stageFake.updateHitbox();
				bgArray[1] = stageFake;
				add(stageFake);
				bgArray[0].alpha = 0;
				bgArray[1].alpha = 0;
		...
		if(curStage == 'SenHouse') {
			var rain:FlxSprite = new FlxSprite();
			rain.frames = Paths.getSparrowAtlas('SENAnimatedAssets/WhiteRain');
			rain.animation.addByPrefix('raining', 'falling', 12, true);
			rain.animation.play('raining');
		...
		if(curStage == 'Glitchy') {
			var rain:FlxSprite = new FlxSprite();
			rain.loadGraphic(Paths.image('SENAnimatedAssets/DangerFlicker'));
			rain.scrollFactor.set(0,0);
			rain.scale.set(.7, .7);
			rain.screenCenter();
			rain.alpha = 0;
		...
		DrawOver = new FlxSprite().makeGraphic(FlxG.width*3, FlxG.height*3, FlxColor.BLACK);
		DrawOver.scrollFactor.set(0,0);
		DrawOver.scale.set(5,5);
		DrawOver.updateHitbox();
		DrawOver.screenCenter();
		DrawOver.alpha = 0;
		// "GLOBAL" SCRIPTS
		#if LUA_ALLOWED
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			if (curStage == "SenHouse"){
				add(bgArray[2]);
				bgArray[2].cameras = [camHUD];
				add(DrawOver);
				DrawOver.cameras = [camHUD];
			}
			if (curStage == "Glitchy"){
				add(DrawOver);
				DrawOver.cameras = [camHUD];
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			case 'Glitchy':
				...
				{
					case 0:
						FlxTween.tween(DrawOver, {alpha: 0}, 4);
					case 179:
						defaultCamZoom = 0.4;
						camGame.maxScrollY = 700;
						camGame.maxScrollX = 1800;
						bgArray[0].alpha = 1;
						bgArray[1].alpha = 1;
					case 308:
						bgArray[3].scale.set(3, 3);
						gf.visible = true;
						gf.alpha = 1;
						bgArray[0].alpha = 0;
						bgArray[1].alpha = 0;
					...
					case 324:
						...
						dad.alpha = 1;
					case 372:
						var red:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.RED);
						red.scale.set(3, 3);
				...
				switch(curStep){
					case 664:
						DrawOver.alpha = 1;
					case 715:
						FlxFlicker.flicker(DrawOver, 1, 0.1, false);
				}
			case 'SenHouse':
				switch(curBeat)
				{
					case 0:
						FlxTween.color(boyfriend, 3, boyfriend.color, 0xFFFFFFFF);
						FlxTween.color(dad, 3, dad.color, 0xFFFFFFFF);
						FlxTween.tween(bgArray[2], {alpha: .5}, 3);
				}
				switch(curStep)
				{
					case 566:
						DrawOver.alpha = 1;
						FlxFlicker.flicker(DrawOver, 3, 0.15, true);
					case 580:
						DrawOver.alpha = 0;
						bgArray[2].alpha = 1;
						FlxTween.tween(bgArray[2], {alpha: .5}, 3);
					case 648:
						FlxFlicker.flicker(bgArray[1], 1, 0.1, false);
						FlxFlicker.flicker(bgArray[0], 1, 0.1, false);
						FlxFlicker.flicker(bgArray[2], 1, 0.1, false);
					case 736:
						bgArray[0].alpha = 1;
						bgArray[1].alpha = 1;
						bgArray[0].visible = true;
						bgArray[1].visible = true;
						bgArray[2].visible = true;
						bgArray[2].alpha = .3;
				}
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'Pulse':
				bgArray[2].alpha = 1;
				FlxTween.tween(bgArray[2], {alpha: 0}, .2);
			case 'Hey!':
				var value:Int = 2;
